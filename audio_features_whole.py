...
import sys
import pickle
sys.path.append('D:/pywork/icassp2022-depression-main/DepressionCollected/Classification')
sys.path.append('D:/pywork/icassp2022-depression-main/DepressionCollected/Classification/vggish')
...
def extract_features(number, audio_features, targets, path):
    ...
    wave_data1 = np.frombuffer(positive_file.readframes(nframes1), dtype=np.short).astype(np.float32)
    len1 = nframes1 / sr1
    ...
    wave_data2 = np.frombuffer(neutral_file.readframes(nframes2), dtype=np.short).astype(np.float32)
    len2 = nframes2 / sr2
    ...
    wave_data3 = np.frombuffer(negative_file.readframes(nframes3), dtype=np.short).astype(np.float32)
    len3 = nframes3/sr3
